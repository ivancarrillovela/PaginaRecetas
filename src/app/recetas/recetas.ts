    ...
    this.vm$ = combineLatest([datosServidor$, this.filtroNombre$, this.filtroEstrellas$]).pipe(
      map(([recetas, nombre, estrellas]) => {
        const filtradas = recetas.filter(r => {
          const coincideNombre = r.nombre.toLowerCase().includes(nombre);
          
          // LÓGICA CAMBIADA: Filtro exacto
          // Si estrellas es 0, mostramos todas.
          // Si no, comparamos la parte entera de la puntuación (4.8 -> 4) con el filtro.
          const coincideEstrellas = estrellas === 0 || Math.floor(r.puntuacion) === estrellas;

          return coincideNombre && coincideEstrellas;
        });
